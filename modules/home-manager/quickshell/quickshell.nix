{ config, pkgs, ... }:
{
  home.file."${config.home.homeDirectory}/.config/quickshell".source =
    if pkgs.stdenv.hostPlatform.system == "aarch64-darwin" then config.lib.file.mkOutOfStoreSymlink "${config.home.homeDirectory}/nixos-config/modules/home-manager/quickshell/mac-config"
