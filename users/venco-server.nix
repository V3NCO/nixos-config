{ pkgs, ... }:
{
  users.users.venco = {
    isNormalUser = true;
    initialPassword = "password";
    extraGroups = [
      "wheel"
      "docker"
      "incus-admin"
      "libvirtd"
    ]; # Enable ‘sudo’ for the user.
    packages = with pkgs; [
      tree
    ];
    openssh.authorizedKeys.keys = [
      # Stardust SSH Key
      "ecdsa-sha2-nistp256 AAAAE2VjZHNhLXNoYTItbmlzdHAyNTYAAAAIbmlzdHAyNTYAAABBBONQ8znwJEINzYqw077cPeQGdxuNojS1q8v2PXFbv3hFPCWEgv+Z8J+xOQHJXDaM+48Bd8g67QkKADxQtFkJHak= v3nco@v3nco.dev"
      # Sentinel SSH
      "sk-ssh-ed25519@openssh.com AAAAGnNrLXNzaC1lZDI1NTE5QG9wZW5zc2guY29tAAAAIHFDwAnZIKGTxA8g0o0sQ54WP9ZCLeACy8OHrLG8OADNAAAABHNzaDo="
      # Padlock
      "sk-ssh-ed25519@openssh.com AAAAGnNrLXNzaC1lZDI1NTE5QG9wZW5zc2guY29tAAAAINp2GvkCu0I1fBPAeP1ttNju7h7gYWNE01AMh4mV/oE3AAAABHNzaDo="
      # Guardian GPG
      "ssh-rsa  openpgp:0x148001EA"
      "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIO7+obmR6DqfhX9gOfuz+pt4YF6RzTiJ91fAdufsd9e5 quickemu@QUICKEM-SFNIRKG"
    ];
  };
}
